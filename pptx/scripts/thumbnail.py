...
def get_soffice_path() -> str:
    if platform.system() == "Windows":
        ...
        common_paths = [
            program_files / "LibOffice" / "program" / "soffice.exe",
            program_files_x86 / "LibOffice" / "program" / "soffice.exe",
...
def convert_to_images(pptx_path: Path, temp_dir: Path) -> list[Path]:
    ...
    pdftoppm_exe = get_pdftoppm_path()
    result = subprocess.run(
        [
            pdftoppm_exe,
            "-jpeg",
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Image conversion failed: {result.stderr.strip()}")
