...
Agent 遇到需求 → 组合复合词 → 没有现成实现则自实现（import 库写代码）。

Skill 通过迭代演化：高频路径被固化为库函数，常见错误沉淀为约束规则，文档膨胀后拆分文件。这是 skill 作者在复盘中做的事，不是 Agent 的运行时行为。
...
**`-` 只是视觉连接符**，没有语法意义。复合词不是被机器解析的 token，是 LLM 写给自己的语义标签。同一对话中用过一次，LLM 就能从上下文认出来，不需要字符串精确匹配。
...
```
识别需求 → 组合复合词 → 检查库是否有现成函数 → 没有 → import 底层库 + 写调用代码 → 执行
```
...
关键是**犯错后有正向反馈**：
- 错误信息 → agent 理解为什么错 → 修正组合或参数 → 重新执行
- 不追求防呆，追求快速修正
...
- **从小开始**。只写原子词表 + 底层库 + 2-3 个示例。不预测所有未来需求。
- **留生长空间**。保持章节模块化，未来拆文件时能干净地抽出。
...
- [ ] 高频复合词是否已固化为库函数？（反复自实现 = 需要固化）
- [ ] 常见错误是否有约束规则？（反复犯错 = 需要补充安全规则）
- [ ] SKILL.md 是否超过 500 行？（超过 → 拆到 ref.md / guide.md / lessons.md）
- [ ] 是否有子领域操作足够独立，应分裂为单独 skill？
